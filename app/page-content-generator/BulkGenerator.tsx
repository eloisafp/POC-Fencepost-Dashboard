...
import {
  createMasterDoc, writeContentToTab,
} from './googleDocsWriter'
...
// Lives outside React so state survives route-level navigation (component unmounts).
// Background async generation continues writing to this store; when the user
// navigates back the component re-registers and immediately sees live state.

type GenStore = {
  groups:      ClientGroup[]
  templates:   PageTemplate[]
  notify:      ((g: ClientGroup[]) => void) | null
  abortFlags:  Map<string, { shouldStop: boolean }>
  controllers: Map<string, AbortController>
}
const _genStore: Partial<Record<Mode, GenStore>> = {}
function getGenStore(mode: Mode): GenStore {
  return (_genStore[mode] ??= {
    groups: [], templates: [], notify: null,
    abortFlags: new Map(), controllers: new Map(),
  })
}
...
async function generateRow(
  ...
  if (!accessToken) throw new Error('Missing token — cannot write to Google Doc')
  const location   = [row.city, row.state].filter(Boolean).join(', ')
  const middle     = row.keyword || [row.service, location].filter(Boolean).join(' ')
  const dateStr    = new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
  const pageHeader = `${companyName.toUpperCase()} - ${middle} - Page for Review - ${dateStr}`
  const monthYear  = new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })
  const docTitle   = [...[companyName, row.service, row.city, row.state].filter(Boolean), monthYear].join(' - ')
  const docId      = await createMasterDoc(docTitle, folderId, accessToken)
  await writeContentToTab(docId, null, seo, blocks, accessToken, pageHeader)
  return { docUrl: `https://docs.google.com/document/d/${docId}/edit?usp=sharing` }
}
...
function ClientGroupCard({ group, mode, templates, openDrivePicker, onUpdate, onDelete, accessTokenRef, getAccessToken }: {
  group: ClientGroup
  mode: Mode
  templates: PageTemplate[]
  ...
  async function addRowManually() {
    ...
    if (inserted) {
      updateRows(rows => [{ ...newRow, id: inserted.id, pageType: mode, status: 'pending' }, ...rows])
    }
    setNewRow(blankRow)
    setShowAdd(false)
    setAddBusy(false)
  ...
  function handleClientCsv(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    Papa.parse(file, {
      ...
      complete: async (result) => {
        ...
        // Auto-populate folder + website from master client if not already set
        if (!group.folderId) {
          const { data: cd } = await supabase
            .from('master_clients')
            .select('website_url, location_page_folder_url')
            .ilike('client_name', group.companyName)
            .limit(1)
            .single()
          if (cd) {
            const m = (cd.location_page_folder_url ?? '').match(/\/folders\/([a-zA-Z0-9_-]+)/)
            const folderId   = m ? m[1] : ''
            const folderName = folderId ? `${group.companyName} — Location Pages` : ''
            if (folderId) onUpdate(g => ({ ...g, folderId, folderName, websiteUrl: cd.website_url || g.websiteUrl }))
          }
        }
        ...
        if (inserted) updateRows(rows => {
          const existing = new Set(rows.map(r => r.id))
          return [...inserted.filter((r: any) => !existing.has(r.id)).map(dbRowToLocal), ...rows]
        })
      },
    })
    e.target.value = ''
  ...
  async function runGroup(fromIndex = 0) {
    ...
    // Get token HERE — still inside the button-click user gesture, so the OAuth popup is allowed.
    // Inside navigator.locks.request the gesture context is lost and popups get blocked.
    const accessToken = await getAccessToken().catch(() => '')
    if (!accessToken) { alert('Could not connect to Google. Please try clicking Generate again.'); return }

    abortRef.current = false
    const controller = new AbortController()
    abortControllerRef.current = controller
    const genStore = getGenStore(mode)
    const abortFlag = { shouldStop: false }
    genStore.abortFlags.set(group.uid, abortFlag)
    genStore.controllers.set(group.uid, controller)
    ...
    const runLoop = async () => {
      ...
      // accessToken is captured from the outer runGroup scope (fetched before the lock)
      ...
      for (let i = fromIndex; i < currentRows.length; i++) {
        if (abortFlag.shouldStop || abortRef.current) break
        ...
        try {
          const { docUrl } = await generateRow(
            row, group.companyName, group.websiteUrl, group.folderId,
            template?.sections, intakeFormContent, contentGuidelinesContent,
            accessToken || undefined, controller.signal,
          )
          ...
          genStore.abortFlags.delete(group.uid)
          genStore.controllers.delete(group.uid)
          return
        }
        if (i < currentRows.length - 1 && !abortFlag.shouldStop && !abortRef.current) {
      ...
      <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 flex items-center gap-2 flex-wrap">
        ...
        {group.folderName ? (
          <div className="flex items-center gap-1.5 h-8 border border-green-200 bg-green-50 rounded-md px-2.5">
            ...
            <a href={`https://drive.google.com/drive/folders/${group.folderId}`} target="_blank" rel="noopener noreferrer"
              title="Open client folder in Drive"
        ...
        {group.isRunning ? (
          <button onClick={() => {
            ...
            const gs = getGenStore(mode)
            const flag = gs.abortFlags.get(group.uid)
            if (flag) flag.shouldStop = true
            gs.controllers.get(group.uid)?.abort()
            onUpdate(g => ({ ...g, isRunning: false, rows: g.rows.map(r => r.status === 'generating' ? { ...r, status: 'pending' as const, errorMsg: undefined } : r) }))
          }}
            ...
            <div className="flex items-center gap-2">
              ...
              {!group.isRunning && (
                <button onClick={onDelete}
                  className="text-xs px-2.5 h-7 rounded-md border border-red-200 text-red-500 hover:bg-red-50 transition-colors flex items-center gap-1">
...
export default function BulkGenerator({ openDrivePicker, mode, accessTokenRef, getAccessToken }: {
  openDrivePicker: (onSelect: (id: string, name: string) => void) => void
  mode: Mode
  accessTokenRef: React.MutableRefObject<string>
  getAccessToken: () => Promise<string>
}) {
  const [groups,        _setGroupsRaw]    = useState<ClientGroup[]>(() => getGenStore(mode).groups)
  const [templates,     setTemplates]     = useState<PageTemplate[]>(() => getGenStore(mode).templates)
  const [loading,       setLoading]       = useState(() => getGenStore(mode).groups.length === 0)
  const [masterError,   setMasterError]   = useState<string | null>(null)
  const [searchClient,  setSearchClient]  = useState('')

  // Stable setter that persists to the module store so background generation
  // can update state even after this component instance unmounts.
  const setGroups = useCallback((updater: ClientGroup[] | ((g: ClientGroup[]) => ClientGroup[])) => {
    const s = getGenStore(mode)
    const next = typeof updater === 'function' ? updater(s.groups) : updater
    s.groups = next
    s.notify?.(next)
  }, [mode])
  ...
  const loadGroups = useCallback(async () => {
    ...
    getGenStore(mode).templates = tmpl
    setTemplates(tmpl)
    setLoading(false)
  }, [mode, setGroups])

  // Register this component instance as the live listener for the store.
  // On first visit: store is empty → load from Supabase.
  // On navigation back: store has data → restore immediately, skip Supabase reload.
  useEffect(() => {
    const s = getGenStore(mode)
    s.notify = _setGroupsRaw
    if (s.groups.length > 0) {
      _setGroupsRaw(s.groups)
      if (s.templates.length > 0) setTemplates(s.templates)
      setLoading(false)
    } else {
      loadGroups()
    }
  ...
  function addGroup() {
    const defaultTemplate = templates.find(t => t.name === 'Default Template for All')
    setGroups(g => [{
      uid: localUid(), companyName: '', websiteUrl: '',
      folderId: '', folderName: '', templateId: defaultTemplate?.id || '',
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <button onClick={addGroup} className="flex items-center gap-1.5 px-3 h-8 rounded-lg bg-zinc-900 text-white text-xs font-medium hover:bg-zinc-700 transition-colors">
      ...
      {groups.filter(g => !searchClient || g.companyName.toLowerCase().includes(searchClient.toLowerCase())).map(group => (
        <ClientGroupCard key={group.uid} group={group} mode={mode} templates={templates} openDrivePicker={openDrivePicker}
          accessTokenRef={accessTokenRef} getAccessToken={getAccessToken}
